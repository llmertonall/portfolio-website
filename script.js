...
contactForm.addEventListener('submit', function(e) {
    ...
    // FormSubmit.co ile gerçek email gönderimi (EmailJS'e bağımlı değil)
    sendViaFormSubmit(emailData, submitBtn, originalText);
});

// FormSubmit.co ile email gönderme fonksiyonu
function sendViaFormSubmit(emailData, submitBtn, originalText) {
    // FormSubmit.co endpoint'ine POST isteği gönder
    const formData = new FormData();
    formData.append('name', emailData.user_name);
    formData.append('email', emailData.user_email);
    formData.append('subject', `Portföy Sitesi İletişim: ${emailData.user_subject}`);
    formData.append('message', emailData.message);
    formData.append('_next', window.location.href); // Başarılı gönderimden sonra aynı sayfaya dön
    formData.append('_captcha', 'false'); // Captcha'yı devre dışı bırak
    formData.append('_template', 'table'); // Email formatı

    fetch('https://formsubmit.co/mertonall@gmail.com', {
        method: 'POST',
        body: formData
    })
    .then(response => {
    ...
    .catch(error => {
        ...
        // Son çare olarak mailto kullan
        showNotification('Email servisinde sorun var. mailto ile gönderiliyor...', 'warning');
